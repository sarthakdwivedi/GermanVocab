async function initGenderRulesPage(){
  ...
  const targetRules = selectedRule
    ? rules.genderRules.filter(rule => rule.id === selectedRule)
    : rules.genderRules;

  document.getElementById('rulesList').innerHTML = targetRules.map(rule => {
    const linked = allNouns
      .filter(noun => noun.genderRuleId === rule.id)
      .slice(0, 20);
    ...
    `;
  }).join('') || '<p>No gender rule found.</p>';
}

document.addEventListener('DOMContentLoaded', initGenderRulesPage);
